...
class SimulatedAnnealing:
    ...
    def custom_log(self, temperature, iteration_at_temperature, current_score, best_score, t0):
        if iteration_at_temperature == (self.iterations_per_temperature - 1):
            t1 = format_time(time.time() - t0)
            print(
                f"Temperature: {temperature}, Current score: {current_score}, "
                f"Best score: {best_score:8.5f}, Elapsed Time: {t1}, "
    ...
    def solve(self):
        ...
        best_trees = clone_trees(primitive_trees)
        best_score = current_score

        for cooling_step in range(self.nsteps):
            for iteration_at_temperature in range(self.iterations_per_temperature):
                move_type = random.randint(0, len(primitive_trees) + 1)
                if move_type < len(primitive_trees):
                    old_params = self.perturb_tree(primitive_trees[move_type], temperature)
                elif move_type == len(primitive_trees):
                ...
                if self.has_overlap(self.translate(primitive_trees, a, b, self.num_cell)):
                    # repair if overlap
                    if move_type < len(primitive_trees):
                        primitive_trees[move_type].set_params(*old_params)
                    elif move_type == len(primitive_trees):
                        a = old_a
                        b = old_b
                    else:
                    ...
                    self.custom_log(temperature, iteration_at_temperature, current_score, best_score, t0)
                    continue
                else:
                    lattice_trees = self.translate(primitive_trees, a, b, self.num_cell)
...
def evaluate_and_store(tree_list):
    N = len(tree_list)
...
if __name__ == '__main__':
    ...
    def fill_missing_solutions(new_trees, max_n=200):
        ...
        for plot in range(1, max_n + 1):
            # ready plot -> skip
            if plot in new_trees:
                continue

            # all bigger plots
            biggers_M = [available_plot for available_plot in available_plots if available_plot > plot]
            if not biggers_M:
                print("Brak większego plotu, coś nie tak, ustawić na większą siatkę")
                continue
            ...
            print(f"Fill N={plot} using best of M={bigger_M}, side={best_side}")
